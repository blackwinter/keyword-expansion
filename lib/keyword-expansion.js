...
let url = require("sdk/url"), tabs = require("sdk/tabs"), KeywordExpansion = {
  ...
  expandUrl: function(aUrl, aText, aLoc) {
    let ret = function(value) { ret.value = value; }, res =
      aUrl.replace(this.pattern, this.replaceDirective(aUrl, aText, aLoc, ret));
  ...
  replaceDirective: function(aUrl, aText, aLoc, ret) {
    ...
    return function(match, directive, options, compat) {
      ...
      let opt = ke.parseOptions(options),
          res = function(value) { res.value = value; };
      ...
      let esc = opt.escape.toString() !== "false" ?
        encodeURIComponent : function(i) { return i; };

      switch (directive) {
        case "selection":
          aText ? res(esc(aText.trim().replace(/\s+/g, " "))) :
